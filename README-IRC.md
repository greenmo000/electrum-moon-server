IRC is used by Electrum-MOON server to find 'peers' - other Electrum-MOON servers. The
current list can be seen by running:

    electrum-moon-server peers
...
`electrum-moon-server peers` will always return an empty list.

`host` is a fully-qualified domain name (FQDN) of your Electrum-MOON server. It is
...
composing the IRC nickname to identify your server on #electrum-moon.
